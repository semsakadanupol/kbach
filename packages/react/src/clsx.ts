 ...
 * ```
 *
 * The static build-time scanner (vite-plugin/postcss-plugin) already
 * recognizes `clsx(...)` call sites (alongside `cn`/`classnames`/`cx`/
 * `kb`) and extracts every string literal inside them on its own — using
 * this function needs no extra plugin configuration to keep working with
 * the static CSS build.
 *
 * Identical to `@kbach/react-native`'s own `clsx.ts` — deliberately
 * duplicated rather than factored into a shared package, since this is
 * genuinely all there is to it and a whole extra published package for one
 * ~20-line function would be more ceremony than the thing itself.
 */
