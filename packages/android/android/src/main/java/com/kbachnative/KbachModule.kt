 ...
 * (packages/android/specs/NativeKbachModule.ts -> this package's own
 * codegenConfig, discovered via autolinking ->
...
class KbachModule(reactContext: ReactApplicationContext) : NativeKbachModuleSpec(reactContext) {
    ...
    companion object {
        ...
        @JvmStatic
        external fun nativeResolveStyle(classString: String, themeJson: String, colorScheme: String, pressed: Boolean): String
    }
     ...
     */
    override fun resolveStyle(classString: String, themeJson: String, colorScheme: String, pressed: Boolean): String {
        return nativeResolveStyle(classString, themeJson, colorScheme, pressed)
    }
}
