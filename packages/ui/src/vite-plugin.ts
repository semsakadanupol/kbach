...
import { getModifierOrder } from './core/registry';
...
function classifyToken(token: string, responsiveRe: RegExp): string {
  ...
  // Reconstruct the bare utility-value shape the GROUPS regexes below match
  // against, stripping modifiers via the parser rather than a hardcoded
  // regex list — the old `/^(hover|focus|active|group-hover|peer-hover):/`
  // strip only covered 5 of the 40+ modifiers, so e.g. `disabled:opacity-50`,
  // `checked:bg-blue-6`, and `focus-within:ring-2` all fell through to the
  // 'Utilities' catch-all instead of their real category (Effects/Colors/
  // Borders), silently misfiling them in the generated kbach.css.
...
// ── Responsive @media merging ──────────────────────────────────────────────────
//
// buildClassCSSRules() wraps every responsive class's rule in its OWN
// `@media (min-width: Npx) { ... }` — correct for the runtime <style>-sheet
// injection path (each rule is inserted independently), but for the static
// file it means a project with many classes at the same breakpoint (`sm:p-4`,
// `sm:text-lg`, `sm:flex`, …) gets the identical `@media (min-width: 640px)`
// wrapper repeated once per class instead of grouped into one block — real
// projects can end up with dozens of near-identical wrappers back to back.
// This merges them by condition before prettifying, same output CSS, far less
// repetition. Only applied to the "Responsive" group (see classifyToken) —
// other groups' occasional @media wraps (motion-safe:, print:, …) are rarer
// and mixing them in would blur the human-readable category boundaries the
// rest of this formatter is organized around.
const RESPONSIVE_MEDIA_RE = /^@media \(min-width: (\d+)px\) \{ (.+) \}$/;

function mergeResponsiveMediaBlocks(items: { raw: string }[]): string[] {
  const byWidth = new Map<number, string[]>();
  const widthOrder: number[] = [];
  const unwrapped: string[] = [];

  for (const { raw } of items) {
    // A single token can expand to more than one '\n'-joined rule (e.g. a
    // mode-aware color class expanding into separate light/dark rules) —
    // each line is independently checked for its own @media wrapper.
    for (const line of raw.split('\n')) {
      if (!line.trim()) continue;
      const m = RESPONSIVE_MEDIA_RE.exec(line);
      if (!m) { unwrapped.push(line); continue; }
      const width = Number(m[1]);
      if (!byWidth.has(width)) { byWidth.set(width, []); widthOrder.push(width); }
...
export function formatKbachCSS(tokenCSS: Map<string, string>, theme: ThemeConfig, responsiveRe: RegExp): string {
  ...
  // Each group's items carry the modifier-order value (registry.ts's
  // ModifierDef.order, same source of truth the runtime injector uses) so
  // same-group items get sorted by it below — otherwise two same-specificity
  // rules (e.g. `hover:bg-blue-6` and `focus:bg-red-6`, both in "Colors")
  // would land in whatever order the file scanner happened to discover them,
  // making which one wins when both pseudo-states are active at once depend
  // on source-scan order instead of a fixed, predictable priority.
  //
  // Stores the var-substituted RAW css (not yet prettified) — the
  // "Responsive" group below merges same-breakpoint rules into one @media
  // block before prettifying, which needs the compact single-line form
  // buildClassCSSRules actually emits, not prettifyCSS's already-indented
  // multi-line output.
  const groups = new Map<string, { order: number; raw: string }[]>(GROUP_ORDER.map(g => [g, []]));
  ...
  for (const label of GROUP_ORDER) {
    ...
    // Array.prototype.sort is stable (ES2019+) — same-order items keep their
    // original (scan) order relative to each other.
    const sorted = [...items].sort((a, b) => a.order - b.order);
    out.push(`\n/* ${label} */`);
    const rendered = label === 'Responsive'
      ? mergeResponsiveMediaBlocks(sorted).map(prettifyCSS)
      : sorted.map((it) => prettifyCSS(it.raw));
...
export function kbach(userConfigOrOptions?: FrameworkConfig | KbachPluginOptions): Plugin {
  ...
  // The initial scan (scanProjectCssSelectors/initialScan above) only walks
  // includeDirs — without this same boundary, the dev-server watcher below
  // (which fires for ANY project file, not just includeDirs) would index a
  // file's classes during a dev session that a fresh `vite build` never
  // would, silently diverging dev output from production for any edit made
  // outside the configured include dirs.
  function isWithinIncludeDirs(file: string): boolean {
